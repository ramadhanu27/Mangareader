...
(function ($) {
  ...
  function displayBulkResults(data) {
    ...
    $summary.append('<div style="color: #22c55e;"><strong>' + existingCount + "</strong><br>Already Imported</div>");
    $summary.append('<div style="color: #2271b1;"><strong>' + newCount + "</strong><br>New Items</div>");
    ...
    data.items.forEach(function (item, index) {
      ...
      var rowClass = item.exists_in_db ? "mws-row-exists" : "";
      var existsBadge = "";
      var actionButtons = "";

      if (item.exists_in_db) {
        existsBadge =
          '<span style="display: inline-block; padding: 2px 8px; background: linear-gradient(135deg, #22c55e 0%, #16a34a 100%); color: #fff; font-size: 10px; font-weight: 600; border-radius: 12px; margin-left: 8px;">✓ Already Imported</span>';
        actionButtons =
          '<button type="button" class="button button-small mws-view-item" data-index="' +
          index +
          '">View</button> ' +
          '<a href="' +
          (item.existing_edit_url || "#") +
          '" target="_blank" class="button button-small" style="background: #22c55e; border-color: #16a34a; color: #fff;">Edit</a>';
      } else {
        actionButtons =
          '<button type="button" class="button button-small mws-view-item" data-index="' + index + '">View</button> ' + '<button type="button" class="button button-small mws-import-item" data-index="' + index + '">Import</button>';
      }
      ...
      $tbody.append(
        '<tr data-index="' +
          ...
          rowClass +
          '" style="' +
          (item.exists_in_db ? "background: rgba(34, 197, 94, 0.08);" : "") +
          '">' +
          '<td class="check-column"><input type="checkbox" class="mws-result-check"' +
          (item.exists_in_db ? " disabled" : "") +
          "></td>" +
          "<td>" +
          coverImg +
          "</td>" +
